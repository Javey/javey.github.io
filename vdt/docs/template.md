...
理解上述规则后，下面的概念也就一目了然了：__其实都是在大括号中`{}`书写上述数据类型的表达式__

## 变量

输出一个变量，需要保证取值合法，即变量已定义，访问对象不能为`null | undefined`
...
* 浏览器渲染时，有多种方法来加载父模板
    ...
    但从性能上考虑，更推荐前端使用模块加载器`RequireJs`等工具来加载编译好的文件。
        ...
        * 在生产环境下，可以使用[gulp-vdt][1]插件将`Vdt`提前编译成`js`文件
        ...
        ```js
        // 我们无需关系模板自己的依赖，即child对layout的依赖，只需要申明自己的依赖即可
    ...
    3. 可以借助`webpack`打包工具，将所有依赖整合成单文件。`Vdt`提供了配套工具[vdt-loader][2]来加载`Vdt`模板文件
        ...
        此时在子模板中，注意加上`vdt`文件后缀来匹配到vdt-loader中的`/\.vdt$/`
...
* `<b:block>` 定义往父模板哪个区域填充内容，其中`block`为父模板中定义的`block`名称。需要注意以下规则：
    ...
    * 如果存在嵌套，并且被该`block`嵌套的`block`在父模板中找不到对应`block`名，则该`block`将被忽略
        ...
        ```
    * 如果父模板定义的`block`没有被子模板填充，则该`block`的内容将会直接输出
        ```jsx
        var layout = require('./layout.vdt');
...
在使用`<t:template>`时，通过指定属性，可以传递数据给被包含的模板，同样需要注意 __Vdt不会检测js合法性__，
因此需要手动保证取值合法`typeof data === 'undefined'`
