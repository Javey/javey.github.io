...
},{"dup":5,"individual/one-version":22}],21:[function(require,module,exports){
...
function removeNode(domNode, vNode) {
    ...
    //vNode.beforeDestroy && vNode.beforeDestroy(domNode);
    destroyWidget(domNode, vNode);

    if (parentNode) {
        //domNode.addEventListener('webkitTransitionEnd', function(e) {
        //    console.log(2222)
        //    e.stopPropagation();
...
function widgetPatch(domNode, leftVNode, widget, renderOptions) {
    ...
    if (!updating) {
        destroyWidget(domNode, leftVNode, newNode)
    }

    if (parentNode && newNode !== domNode) {
        //leftVNode.beforeDestroy && leftVNode.beforeDestroy(domNode);
        parentNode.replaceChild(newNode, domNode)
    }
...
function destroyWidget(domNode, w, newNode) {
    if (typeof w.destroy === "function" && isWidget(w)) {
        w.destroy(domNode, newNode)
    }
}
