...
为了方便大家阅读连贯性，这里也会介绍一些常见的模板语法。

> Vdt是一个基于虚拟DOM的模板引擎，在JSX语法的基础上，新增了一些指令来实现
> 条件判断、循环、继承等功能。得益于JSX的灵活性，你可以让模板做更多的事情。
> 所以它并非一个纯做表现的模板引擎，你可以将表现逻辑写入模板中。笔者的观点是：
> 业务逻辑写入组件，表现逻辑写入模板，这样可以分担组件逻辑过分膨胀的问题。
...
style属性也可以既支持字符串，又可以支持对象赋值。用对象赋值时，需要使用属性名的驼峰命名方式。
...
对于组件，会传入组件的实例作为参数；对于元素，会传入dom作为参数。在组件或元素被销毁时，该函数会
被再次调用，传入`null`作为参数。
...
对于循环渲染的时候（下面会将如何循环渲染），我们可以使用对象保存引用
...
<div>
    <Component 
        v-for={self.get('list')}
        ref={function(i) { 
            // 如果不存在components，则初始化空对象
            if (!self.compoents) self.compoents = {};
            // 使用v-for循环的key作为键名
...
key属性用于给元素提供一个唯一的标识，在兄弟元素改变时，能够快速元素的增删改和移动。
在渲染列表时，虽然没有强制提供`key`属性，但提供为列表提供`key`属性是个良好的习惯。
...
此时给input设置`key`，则可以解决问题，因为`key`不同，会删除后重建。
...
> JS表达式就是可以作为右值的式子，简单地说：能够赋给一个另一个变量的式子
...
> Vdt模板支持访问全局变量。这会在你项目全局引入了`lodash`等工具函数库时，
> 可以直接在模板中方便地使用它们而无需注入模板。但所有的便利都是双刃剑，
...
> `v-else-if & v-else`必须跟在`v-if`元素后面，否则会引起编译报错（空白字符除外）。

## 循环渲染

渲染渲染涉及三个指令：`v-for & v-for-key & v-for-name`。
...
循环是否展示。并且你可以在`v-if`使用`v-for`提供的形参`key & value`来进行条件判断。

# 模板继承

Vdt提供了模板继承功能，能够让我们定义可被服用的模板骨架。该功能涉及以下标签语法：
* `<t:template>` 子模板声明要继承的父模板`template`
* `<b:block>` 子模板声明要填充到父模板的定义的内容区域`block`中
...
在子模板中`<b:block>`中，我们可以调用`parent()`拿到父模板定义的内容，插入到想要的位置。
